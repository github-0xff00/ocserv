...
static void certificate_check(main_server_st *s, struct vhost_cfg_st *vhost)
{
...
unsigned usage;
	...
	if (vhost->perm_config.cert_size > 1)
		return;

	if (gnutls_url_is_supported(vhost->perm_config.cert[0]) == 0) {
		/* no URL */
		ret = gnutls_load_file(vhost->perm_config.cert[0], &data);
		if (ret < 0)
			return;
...
void tls_load_files(main_server_st *s, struct vhost_cfg_st *vhost)
{
	...
	if (last_access != 0) {
		for (i=0;i<vhost->perm_config.key_size;i++) {
			if (need_file_reload(vhost->perm_config.cert[i], last_access) != 0) {
				need_reload = 1;
				break;
			}
		}

		if (need_file_reload(vhost->perm_config.ca, last_access) ||
		    need_file_reload(vhost->perm_config.config->ocsp_response, last_access) ||
		    need_file_reload(vhost->perm_config.dh_params_file, last_access)) {
			need_reload = 1;
		}
...
void tls_reload_crl(main_server_st* s, struct vhost_cfg_st *vhost, unsigned force)
{
	...
	if (vhost->perm_config.config->cert_req != GNUTLS_CERT_IGNORE && vhost->perm_config.config->crl != NULL) {
		if (need_file_reload(vhost->perm_config.config->crl, last_access) == 0) {
