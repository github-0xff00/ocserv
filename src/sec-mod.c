...
static int load_keys(sec_mod_st *sec, unsigned force)
{
	...
	list_for_each(sec->vconfig, vhost, list) {
		...
		for (i = 0; i < vhost->perm_config.key_size; i++) {
			if (need_file_reload(vhost->perm_config.key[i], vhost->last_access) != 0) {
				need_reload = 1;
				break;
			}
		}
		 ...
		 */
		if (sec->key == NULL) {
			sec->key_size = vhost->perm_config.key_size;
			sec->key = talloc_zero_size(sec, sizeof(*sec->key) * vhost->perm_config.key_size);
			if (sec->key == NULL) {
