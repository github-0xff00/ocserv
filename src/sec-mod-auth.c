...
void sec_auth_init(struct vhost_cfg_st *vhost)
{
	...
	for (i=0;i<vhost->perm_config.auth_methods;i++) {
		if (vhost->perm_config.auth[i].enabled && vhost->perm_config.auth[i].amod && vhost->perm_config.auth[i].amod->global_init)
			vhost->perm_config.auth[i].amod->global_init(pool, vhost->perm_config.auth[i].additional);
	}

	if (vhost->perm_config.acct.amod && vhost->perm_config.acct.amod->global_init)
		vhost->perm_config.acct.amod->global_init(pool, vhost->perm_config.acct.additional);
}
...
int handle_secm_session_open_cmd(sec_mod_st *sec, int fd, const SecmSessionOpenMsg *req)
{
	...
	if (e->vhost->perm_config.acct.amod != NULL && e->vhost->perm_config.acct.amod->open_session != NULL && e->session_is_open == 0) {
		ret = e->vhost->perm_config.acct.amod->open_session(e->auth_type, &e->acct_info, req->sid.data, req->sid.len);
		if (ret < 0) {
			e->status = PS_AUTH_FAILED;
...
int handle_sec_auth_stats_cmd(sec_mod_st * sec, const CliStatsMsg * req, pid_t pid)
{
	...
	e->vhost->perm_config.acct.amod->session_stats(e->auth_type, &e->acct_info, &totals);
...
int set_module(sec_mod_st * sec, vhost_cfg_st *vhost, client_entry_st *e, unsigned auth_type)
{
	...
	for (i=0;i<vhost->perm_config.auth_methods;i++) {
		if (vhost->perm_config.auth[i].enabled && (vhost->perm_config.auth[i].type & auth_type) == auth_type) {
			...
			seclog(sec, LOG_INFO, "using '%s' authentication to authenticate user "SESSION_STR, vhost->perm_config.auth[i].name, e->acct_info.safe_id);
			return 0;
		}
	}
...
void sec_auth_user_deinit(sec_mod_st *sec, client_entry_st *e)
{
	...
	if (vhost->perm_config.acct.amod != NULL && vhost->perm_config.acct.amod->close_session != NULL && e->session_is_open != 0) {
		vhost->perm_config.acct.amod->close_session(e->auth_type, &e->acct_info, &e->saved_stats, e->discon_reason);
	}
